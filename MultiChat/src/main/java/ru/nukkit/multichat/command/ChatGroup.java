...
@CmdDefine(command = "chat", alias = "multichat", subCommands = {"group|groupformat", "chat|name|tag|nametag", "\\S+"}, permission = "multichat.config.chatgroup", description = Message.CHAT_SET, allowConsole = true)
public class ChatGroup extends Cmd {
    ...
    public boolean execute(CommandSender sender, Player player, String[] args) {
        ...
        if (!MultiPerms.isGroupExist(group)) Message.CHAT_GROUP_NOTEXIST.print(sender, group);
        return true;
    }
}

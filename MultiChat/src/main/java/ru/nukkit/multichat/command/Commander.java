    ...
    public static boolean execute(CommandSender sender, String cmdLabel, String[] args) {
        for (Cmd cmd : commands) {
            ...
            if (cmd.executeCommand(sender, args))
                return Message.debugMessage("Perintah tidak dieksekusi:", sender.getName(), cmdLabel, cmd.toString(), new ArrayList<String>(Arrays.asList(args)).toString());
        }
        Message.debugMessage("Perintah yang dijalankan:", sender.getName(), cmdLabel, new ArrayList<String>(Arrays.asList(args)).toString());

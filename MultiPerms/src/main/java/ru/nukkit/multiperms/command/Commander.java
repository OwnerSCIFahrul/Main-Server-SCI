...
import ru.nukkit.multiperms.command.group.*;
import ru.nukkit.multiperms.command.perm.*;
import ru.nukkit.multiperms.command.user.*;
...
public class Commander {
    ...
    public static boolean addNewCommand(Class<? extends Cmd> cmd) {
        try {
            ...
            Message.debugMessage("Gagal mendaftar perintah: ", cmd.getName());
            return false;
        }
    }
    ...
    public static void printHelp(CommandSender sender, int page) {
        ...
        Paginator.printPage(sender, helpList, Message.HLP_TITLE.getText('e', '6', (plugin.getName() + " " + plugin.getDescription().getVersion()))
                , "[%1% / %2%]", "tidak ada bantuan", page, pageHeight, false);
    }
    ...
    public static boolean execute(CommandSender sender, String cmdLabel, String[] args) {
        for (Cmd cmd : commands) {
            ...
            if (cmd.executeCommand(sender, args))
                return Message.debugMessage("Perintah yang dijalankan:", sender.getName(), cmdLabel, cmd.toString(), new ArrayList<>(Arrays.asList(args)).toString());
        }
        Message.debugMessage("Perintah tidak dieksekusi:", sender.getName(), cmdLabel, new ArrayList<>(Arrays.asList(args)).toString());

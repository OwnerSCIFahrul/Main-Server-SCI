...
public class MainClass extends PluginBase {
    ...
    private static String DEFAULT_TIMEZONE = "Asia/Jakarta";
    ...
    public void onEnable() {
        ...
        if(version == null || Integer.parseInt(version) < CUSTOM_MESSAGES_VERSION) {
            ...
            if(!newConfig.load(this.getResource("messages.properties"))) {//failed
                Server.getInstance().getLogger().error("Tidak dapat memuat konfigurasi baru.");
        ...
        Map<String, String> cmdDes = new LinkedHashMap<String, String>(){{
            put("addjail", MainClass.this.getCustomMessage("command.otu.description"));
            put("deljail", MainClass.this.getCustomMessage("command.unotu.description"));
            put("setmodejail", MainClass.this.getCustomMessage("command.runa.description"));
            put("setspawnjail", MainClass.this.getCustomMessage("command.otup.description"));
            put("listjail", MainClass.this.getCustomMessage("command.otulist.description"));
            put("checkplayerjail", MainClass.this.getCustomMessage("command.otuser.description"));
            put("setreasonjail", MainClass.this.getCustomMessage("command.otur.description"));
        }};

        Map<String, Map<String, CommandParameter[]>> cmdParams = new LinkedHashMap<String, Map<String, CommandParameter[]>>(){
            {
                put("addjail", new LinkedHashMap<String, CommandParameter[]>(){
                    {
                        put("addjail", new CommandParameter[]{
                                new CommandParameter("player", CommandParameter.ARG_TYPE_RAW_TEXT, false)
                        });
                    }
                });
                put("deljail", new LinkedHashMap<String, CommandParameter[]>(){
                    {
                        put("deljail", new CommandParameter[]{
                                new CommandParameter("player", CommandParameter.ARG_TYPE_RAW_TEXT, false)
                        });
                    }
                });
                put("setmodejail", new LinkedHashMap<String, CommandParameter[]>(){
                    {
                        put("setmodejail", new CommandParameter[]{
                                new CommandParameter("player", CommandParameter.ARG_TYPE_RAW_TEXT, false)
                        });
                    }
                });
                put("setspawnjail", new LinkedHashMap<String, CommandParameter[]>(){
                    {
                        put("setspawnjail", new CommandParameter[]{
                ...
                put("listjail", new LinkedHashMap<String, CommandParameter[]>(){
                    {
                        put("listjail", new CommandParameter[]{
                ...
                put("checkplayerjail", new LinkedHashMap<String, CommandParameter[]>(){
                    {
                        put("checkplayerjail", new CommandParameter[]{
                                new CommandParameter("player", CommandParameter.ARG_TYPE_RAW_TEXT, false)
                        });
                    }
                });
                put("setreasonjail", new LinkedHashMap<String, CommandParameter[]>() {
                    {
                        put("setreasonjail", new CommandParameter[]{
                                new CommandParameter("player", CommandParameter.ARG_TYPE_RAW_TEXT, false),
    ...
    public void updateOtuListTo200() {//update otulist from 1.0.6 to 2.0.0
        ...
        this.getLogger().notice("Sedang memperbarui plugin Jail");

        File file = new File(this.getDataFolder(), "list.yml");

        if(!file.exists() || file.isDirectory()) {
            this.getLogger().notice("Tidak dapat menemukan file Jail baru. jalankan:" + file.getAbsolutePath());
            return;
        }
        ...
        this.getLogger().notice("Mohon Plugin Ini Saling Berbagi Dan Manfaatkanlah");
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (cmd.equals("addjail")) {
        ...
        } else if (cmd.equals("setmodejail")) {
            ...
            this.saveList();
            this.updateActivePlayers();
        } else if (cmd.equals("setspawnjail")) {
            Position pos;
        ...
        } else if (cmd.equals("checkplayerjail")) {
            if (args.length <= 0) {
                this.sendCustomMessage(sender, "command.notEnoughParam");
                return true;
            }
        ...
        } else if (cmd.equals("setreasonjail")) {
            if (args.length <= 1) {
                this.sendCustomMessage(sender, "command.notEnoughParam");
                return true;
            }

package Fahrul8676.NoteBlockAPI;
...
public class Interpolator {
    ...
    public static double[] interpLinear(double[] x, double[] y, double[] xi) throws IllegalArgumentException {
        ...
        if (x.length != y.length) {
            throw new IllegalArgumentException("Panjang X dan Y harus sama");
        }
        if (x.length == 1) {
            throw new IllegalArgumentException("X harus mengandung lebih dari satu nilai");
        }
        ...
        for (int i = 0; i < x.length - 1; i++) {
            dx[i] = x[i + 1] - x[i];
            if (dx[i] == 0) {
                throw new IllegalArgumentException("X harus montotonic. Duplikat " + "nilai x ditemukan");
            }
            if (dx[i] < 0) {
                throw new IllegalArgumentException("X harus disortir");
            }
            dy[i] = y[i + 1] - y[i];
            slope[i] = dy[i] / dx[i];
    ...
    public static double interpLinear(double[] xy, double xx) {
        if (xy.length % 2 != 0) {
            throw new IllegalArgumentException("XY harus dibagi dua.");
        }

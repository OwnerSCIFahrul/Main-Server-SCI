package Fahrul8676.servermusic.resourcepack;
...
public class MusicResourcePack implements ResourcePack {
    ...
    public MusicResourcePack(String md5, byte[] data) throws Exception {
        ...
        md5 = md5.trim();
        Preconditions.checkArgument(md5.length() == 32, "MD5 tidak sah");
        md5 = md5.toLowerCase();
        Preconditions.checkNotNull(data, "data");
        Preconditions.checkArgument(data.length > 0, "Data tidak sah");
        ...
        try (ByteArrayOutputStream baos = new ByteArrayOutputStream(); ZipOutputStream zos = new ZipOutputStream(baos)) {
            ...
            builder.append("{\"content\":[{\"path\":\"manifest.json\"},{\"path\":\"sounds/sound_definitions.json\"},{\"path\":\"sounds/servermusic/");
            builder.append(md5);
            builder.append(".ogg\"}]}");
            ...
            builder.append("{\"servermusic.");
            builder.append(md5);
            builder.append("\":{\"category\":\"music\",\"sounds\":[\"sounds/servermusic/");
            builder.append(md5);
            builder.append("\"]}}");
            buffer = builder.toString().getBytes();
            zos.write(buffer, 0, buffer.length);

            zos.putNextEntry(new ZipEntry("sounds/servermusic/" + md5 + ".ogg"));
